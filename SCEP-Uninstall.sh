...
launchctl asuser "$uid" chroot -u "$uid" / launchctl unload -w /Library/LaunchAgents/com.microsoft.scep_gui.plist
...
echo "Removing installed items"

rm -rf /Library/Application\ Support/Microsoft/scep
rm -rf /Applications/System\ Center\ Endpoint\ Protection.app/

echo "Removing Package Recepts"
RECEIPTLIST=$(pkgutil --pkgs / | grep systemCenterEndpointProtection)

for pkg in $RECEIPTLIST; do
